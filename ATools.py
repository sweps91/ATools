...
def info():
    ...
    list_of_funcs = ["tips() = Shortcut tips",
                    ...
                    "find_in_html(html_data, find, attrs=None, stringed=False) = Searching data in the html code",
                    ...
                    "find_longest(list_of_values) = Find the longest item in the list",
                    "find_index(dataset, searching_name, labels=True) = Search the name in labels or in columns or rows",
                    "picker(list_of_values, start, end, skip) = Pick data from the list - for index in range (start, end, skip)",
                    "slicer(list_of_values, start, end) = Slice data for each value in the list",
                    "nice_data(list_of_values, sign, second_sign=None, on_index=1, break_index=2) = Clear data from larger text strings (eg. in html)",
                    "types(list_of_lists, summary=True, localized=None, reshaped=False) = Explore data types in the lists",
                    "fibonacci_number(stop_num) = Function for counting fibonacci number",
                    "timeit(function_to_measure) = function to measure time",
                    "owid_world_map_data = working with data from our world in data (owid)",
                    "find_trend(team_place_list) = Find the trend - count avg from 5 values in the list",
                    "html_source(url) = paste source html - it returns html code",
                    "html_selector(html_data, select, text=False, data_index=0) = paste html data - returns selected element",
                    "print_nice_numbers(list_of_numbers) = print list of numbers with separator",
                    "time_time(start_time) = measure time of execution",
...
def search_html(url, find="div", attrs={'class':'container'}, selected=None, data_format="html"):
    """Searching for data in the html page.
    Find your dataset and then you can use fn find_in_html().
...
def find_in_html(html_data, find="div", attrs=None, stringed=False):
    """Find data in the html code. Find html dataset in fn search_html().
    div and attrs can be None.

    examples:
    find="div", attrs={"class": "table-responsive"}
    find=None, attrs={"data-value": ""}

    data = soup.find_all(find, attrs)

    You can return data in html or in string.
    """
    from bs4 import BeautifulSoup

    soup = BeautifulSoup(f"{html_data}")
    data = soup.find_all(find, attrs)
...
def find_longest(list_of_values):
    ...
    result = (sorted(len_list))
    result = result[-1]

    return result

def find_index(dataset, searching_name, labels=True):
    """Search the name in labels if labels = True
    or in all dataset (columns or row) if labels = False"""

    if labels == True:
        result = dataset.index(searching_name)
        return result

    else:
        index = 0
        for data in dataset:
            if searching_name in data:
                x = data.index(searching_name)
                break
...
def picker(list_of_values, start, end, skip):
...
def slicer(list_of_values, start, end):
    """Slice data for each value in the list.
    Data = value[start : end]
    start and/or end can be None"""
...
def nice_data(list_of_values, sign, second_sign=None, on_index=1, break_index=2):
    """Clear data from larger text strings (eg. in html).

    sign = set sign for data reading (">", "/")
    second_sign = you can use second sign
    on_index = after which index you want data read
    break_index = after which index you want break the code

    if l == sign or l == second_sign:
                    index += 1
    """
    data = []
...
def types(list_of_lists, summary=True, localized=None, reshaped=False):
    """Explore data types in the lists.
    Return types.

    summary print out number of each type in each list
    localized print out index possitions of desired type
    localized = int (or: float, str, bool)
    reshape = switch rows to columns and vice versa
    """
    
    data = []

    if reshaped == True:
        list_of_lists = reshape(list_of_lists)
    ...
    if summary == True:
        ...
        for d in data:
            ...
            print(f"[List {index}] | int - {integers} | float - {floats} | str - {strings} | bool - {booleans} |")
    ...
    if localized:
        ...
        for list_index in range (len(data)):
            ...
            for item_index in range (len(data[list_index])):
                if data[list_index][item_index] == localized:
                    indexes.append(item_index)

            print(f"[List {list_index}] | indexes: {indexes} \n")


    return data

def fibonacci_number(stop_num):
    """
    function for counting fibonacci number
    """
    stop_num = int(stop_num)
    print(dash)
    print("Stop number is: ", stop_num)
    print(dash)
    ...
    while num <= stop_num:
        ...
        print(dash)
        print(len(fib_list), "numbers in the list")
        print(dash)

def timeit(function_to_measure):
...
def owid_world_map_data(dataframe, list_of_owid_countries, wanted_column, wanted_year, result_devided=1,
                            rounded=2, additional_text=None, additional_text_for_none_rows=None, owid_countries_column="Entity", year_column="Year"):
    """
    fn for working with data from our world in data (owid), using pandas

    dataframe - source dataset from owid (eg. csv)
    list_of_owid_countries - list of countries (names used by owid)
    wanted_column - target column (where desired data)
    wanted_year - for which year data you want
    result_devided = result devided when numbers too big
    rounded = rounded result numbers
    additional_text - additional text for rows where data
    additional_text_for_none_rows - additional text for rows where no data
    owid_countries_column - name of the column where countries names are stored
    year_column - name of the column where years are stored
    ...
    for country in list_of_owid_countries:
        df = dataframe.loc[(dataframe[owid_countries_column] == country) & (dataframe[year_column] == wanted_year)]
        #print(df)

        value = df[wanted_column].values.tolist()
        #value = value[0]

        try:
            value = value[0] / result_devided
    ...
    for v in result_data:
        ...
        if v != None:
            ...
            x = additional_text_for_none_rows
...
def find_trend(team_place_list, inspect=5):
    """Find the trend - count avg from 5 values (inspect=5) in the list
    eg. used in the sport"""

    data = []

    index = 0

    while index + inspect <= (len(team_place_list)):

        sliced = team_place_list[index : index + inspect]
        #print(sliced)
        result = (sum(sliced))
...
def html_source(url):
    """paste source html - it returns html code """
...
def html_selector(html_data, select, text=False, data_index=0):
    """paste html data - returns selected element
        html_data = use fn html_source(url)
        select = eg. "title" or else element
        text = False = returns html, True - returns text
        data index = can be found more elements
        """

    data = html_data.select(select)

    if text == False: return data

    else: return data[data_index].getText()

def print_nice_numbers(list_of_numbers):
    """print list of numbers with separator"""

    for num in list_of_numbers: print(f"{num:_}")

def time_time(start_time):
    """measure time of execution
    create var "start_time" where you want to start measure
    this function take this var like an argument
    place this fn where you want to end measure
    it will print the time of execution
    using time module"""
    import time
...
def timedelta(time="now", days=1_000):
    """count days from time
    time = starting point
    time = "now" - for counting from now moment
    or set different time in format YYYY-MM-DD (eg. 2014-04-12 for 12th April 2014)
    days = how many days you want to count from starting point"""
    ...
    if time == "now":
        ...
        time = time.split("-")
        dt = datetime.datetime(int(time[0]), int(time[1]), int(time[2]), 12, 12, 12)
        days = datetime.timedelta(days=days)
        result = dt + days
        return result

def geojson_linestring(path, list_of_coordinates, name_geojson, reverse_lat_lng=False):
    """Create GeoJSON format
    path - set where to save file
    list_of_coordinates - paste list of coordinates in this form: ['46.651,32.091', '46.662,32.093'] (may also be in float dtype) (each list is list of strings - not one string!)
    name_geojson - set name - it will appear inside the GeoJSON properities
    reverse_lat_lng - reverse positions of coordinates: [lat, lng] or [lng, lat]
    type_geojson - "LineString" (default), also can be: Point, Polygon, MultiPoint, MultiLineString, MultiPolygon
    """
    ...
    list_of_coordinates = change_coordinates_format(list_of_coordinates)
    ...
    with open(f"{path}", "w") as file:
        for line in geojson_format:
            file.write(line)
            file.write("\n")

def geojson_multilinestring(path, list_of_lists_with_coordinates, name_geojson, reverse_lat_lng=False):
    """Create GeoJSON format
    path - set where to save file
    list_of_lists_with_coordinates - paste list of coordinates in this form: 
    [['46.651,32.091', '46.662,32.093'], ['49.651,37.091', '43.662,30.093']] (may also be in float dtype) (each list is list of strings - not one string!)
    name_geojson - set name - it will appear inside the GeoJSON properities
    reverse_lat_lng - reverse positions of coordinates: [lat, lng] or [lng, lat]
    type_geojson - "MultiLineString" (default), also can be: Point, Polygon, MultiPoint, MultiLineString, MultiPolygon
    """
            ...
            for li in coordinates:
                ...
                for co in li:
                    ...
                    if co == li[len(li) - 1]:
                        ...
                        if li == coordinates[len(coordinates) - 1]:
    ...
    list_of_coordinates = change_coordinates_format(list_of_lists_with_coordinates)
